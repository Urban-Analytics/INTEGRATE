...
# Summarize results - Accuracy, F1 vs. parameters
# results_df <- read_csv("restaurant_tuning_all_results_2.csv")
summary_df <- results_df %>%
  mutate(correct = ifelse(GroundTruth == Prediction, 1, 0)) %>%
  group_by(temperature, top_p, top_k, max_tokens, batch_size) %>%
  summarise(
    accuracy = mean(correct, na.rm = TRUE),
    missing = sum(is.na(Prediction)),
    total = n(),
    .groups = "drop"
  )
write_csv(summary_df, "restaurant_tuning_summary_2.csv")
head(summary_df)

# results_df <- read_csv("restaurant_tuning_all_results_2.csv")

# calculate accuracy, percision, recall, F1
metrics_df <- results_df %>%
  mutate(correct = ifelse(GroundTruth == Prediction, 1, 0)) %>%
  group_by(temperature, top_p, top_k, max_tokens, batch_size) %>%
  summarise(
    accuracy = mean(correct, na.rm = TRUE),
    precision = sum(Prediction == 1 & GroundTruth == 1, na.rm = TRUE) /
                sum(Prediction == 1, na.rm = TRUE),
    recall = sum(Prediction == 1 & GroundTruth == 1, na.rm = TRUE) /
             sum(GroundTruth == 1, na.rm = TRUE),
...
p3_a <- metrics_df %>%
  ...
  labs(title = "Heatmap of Accuracy (top_k * max_tokens) by batch_size",
...
p3_b <- metrics_df %>%
  ...
  geom_tile(color = "white") +
  labs(title = "Heatmap of Accuracy (batch_size * top_k), faceted by max_tokens",
...
```{r, eval=FALSE}
# save plots
ggsave("p1_accuracy_batchsize_temperature.png",
       plot = p1,
       width = 8, height = 5, dpi = 300)

ggsave("p2_f1_batchsize_top_p.png",
       plot = p2,
       width = 10, height = 4, dpi = 300)

ggsave("p3_heatmap_accuracy_batchsize_A.png",
       plot = p3_a, width = 14, height = 10, dpi = 300)

ggsave("p3_heatmap_accuracy_batchsize_B.png",
       plot = p3_b, width = 14, height = 8, dpi = 300)
```
